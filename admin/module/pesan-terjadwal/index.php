    ...
    $.get('API/pesan-terjadwal.php', function(data) {
        ...
        data.forEach(function(row, i) {
            html += '<tr>' +
                ...
                '<td>' + row.pesan + '</td>' +
                '<td>' + row.jadwal + '</td>' +
                '<td>' + (row.tiap_bulan == '1' ? 'Ya' : 'Tidak') + '</td>' +
                '<td>' + (row.status || '-') + '</td>' +
                '<td>-</td>' +
                '</tr>';
        });
...
$('#form-pesan-terjadwal').on('submit', function(e) {
    ...
    var tiap_bulan = $('#interval').val() === 'monthly' ? '1' : '0';
    $.post('API/pesan-terjadwal.php', { nomor: nomor, pesan: pesan, jadwal: jadwal, tiap_bulan: tiap_bulan })
        .done(function(res) {
